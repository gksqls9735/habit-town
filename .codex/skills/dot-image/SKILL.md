---
name: dot-image
description: Explicit-only general pixel-art asset skill. Use only when the user directly names `$dot-image`, asks to use the dot-image skill, or requests the dot-image harness by name. Do not auto-select this skill for Habit Town pet growth stages or pet-room assets; use `growth-pixel-character` for those.
---

# Dot Image

Use this skill only when the user explicitly asks for `dot-image` or the dot-image harness. For Habit Town pet growth characters and pet-room visuals, use `growth-pixel-character` instead.

Generate a raster asset, apply the selected visual theme, normalize it onto a fixed pixel grid, and deliver verified outputs from the selected image-category `clean/` folder.
...
For a supplied portrait or panoramic background whose composition must remain intact, pass `--working-grid WIDTHxHEIGHT` with the source aspect ratio. This override is limited to `bg` and `scene` assets.
...
6. Generate one raster PNG with the available image-generation capability. Do not substitute SVG, CSS, emoji, or an icon library.
7. Save the unprocessed image to `<asset-root>/raw/<filename>.png`. Create the directory when needed.
...
10. Inspect the clean PNG at native size and enlarged nearest-neighbor size. Confirm the selected 2D view, parallel projection, silhouette, focal pixels, selective outlines, transparency, and theme remain legible without perspective drift, noise, cropping, or distortion.
11. Report the asset category, raw path, clean path, style profile, asset type, theme, 2D view, dimensions, palette reference when used, harness result, and any visible-size warnings. Composition mode produces a sibling `.composition.json`. The harness appends a machine-readable record to `<asset-root>/clean/dot-harness.log`.
